...
public class TransactionListenerController {
    ...
    public void consume(String encryptedMessage) {
        try {
            String decrypted = AESUtil.decrypt(encryptedMessage, "12345678901234567890123456789012");
            synchronized (latestMessages) {
                if (latestMessages.size() > 20) latestMessages.remove(0);
...
public List<Map<String, Object>> getLatestMessages() {
    ...
    synchronized (latestMessages) {
        for (String msg : latestMessages) {
            try {
                Map<String, Object> parsed = new ObjectMapper().readValue(msg, Map.class);
